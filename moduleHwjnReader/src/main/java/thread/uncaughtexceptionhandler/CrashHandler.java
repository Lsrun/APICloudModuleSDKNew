...
//              MsgPrompt.showMsg(mContext, "程序出错啦", msg+"\n点确认退出");
        ...
        ReadActivity.log("收集设备信息");
        //收集设备信息
        collectCrashDeviceInfo(mContext);
        ReadActivity.log("保存错误报告文件");
        //保存错误报告文件
...
//        sendCrashReportsToServer(mContext);
    ...
    private void postReport(File file) {
        ...
        String boundary = "******";
        try{
//            URL url = new URL(ReadActivity.SERVICE_URL+"/errorLog.php");
            URL url = new URL("http://47.100.49.11/riku/reader/errorLog.php");
...
//            Toast.makeText(this, result, Toast.LENGTH_LONG).show();
    ...
    private String saveCrashInfoToFile(Throwable ex) {
        ...
        try {
            ...
            String fileName = "HwjnCrash-" + date + "-" + time + CRASH_REPORTER_EXTENSION;
            FileOutputStream trace = mContext.openFileOutput(fileName,
                    Context.MODE_PRIVATE);
            mDeviceCrashInfo.store(trace, "");
            trace.flush();
            trace.close();
//            ReadActivity.log(fileName);
            return fileName;
        } catch (Exception e) {
