...
import com.ysy15350.moduleaotaoreader.util.JsonUtils;
...
import thread.uncaughtexceptionhandler.CrashHandler;
...
public class ReadActivity extends MVPBaseActivity<ReadViewInterface, ReadPresenter>
    ...
    private static final int CODE122 = 122;
    ...
    public static String chapterListLastupdate ="";
    ...
    public static double luckymoney = 0;
...
//测试数据
            isDebugStr = intent.getStringExtra("isDebug");

            sign = intent.getStringExtra("sign");
            path = intent.getStringExtra("path");
        }else {
            //
            url="https://www.hanwujinian.com/riku/reader";
            uid="108681";
            aid="8703";
            cid="302052";
            String bookPath="fs://hwjn/article/8703";
            isDebugStr="";
            typeStr="1";
            sign="cUa3dixDR7nHTcX3gZ5SBHfga04SvW0u";
            path = "/storage/emulated/0/UZMap/A6908746897755/hwjn/download/article/8703";
        }
...
//                    startActivityForResult(intent, 789);
    ...
    private List<DirectoryInfo> getDirectoryInfo(String chapterlistJson) {
        if(JsonUtils.isGoodJson(chapterlistJson)) {
            try {
                ...
                if (!CommFun.isNullOrEmpty(chapterlistJson)) {
                    ...
                    if (directoryModel != null) {
                        chapterListLastupdate = directoryModel.getLastupdate();
...
///                    MessageBox.showWaitDialog(ReadActivity.this, "正在读取文章内容...");
        ...
        sb_progress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                pro = (float) ((float)progress / 10000);
            }
...
//                showMsg(str);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        buttonBuy.setVisibility(View.GONE);
//        log("SDFSDFSD:"+requestCode+"***"+resultCode+"***"+Activity.RESULT_OK);
...
//
//                            if (isTCCode == 1) {
//                                textViewTC.setText("关闭看法");
//                            } else {
//                                textViewTC.setText("打开看法");
//                            }
//
//                            /// 保存到本地
//                            saveIsOpen(isTCCode);
...
//                            getTuCaoChange();
                ...
                case CODE122:
                    try {
                        ...
                        if (status == 1) {
                            luckymoney = json.optDouble("data");
                            log("阅读币余额:"+luckymoney);
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    log("llllllll:"+bookList.toString());
                    break;
            }
        }
    };
...
//                            stringBuilder.append(String.format("\r\n%s\r\n", chapterInfo.getChaptername()));
                            if(isTCCode == 1) {
//                                contentArray[0] = contentArray[0].replace("####{flag:0}####","####{flag:0}########{flag:1}########{flag:0}####");
                                stringBuilder.append(String.format("\r\n%s\r\n##{0}", chapterInfo.getChaptername()));
                            }else{
                                stringBuilder.append(String.format("\r\n%s\r\n", chapterInfo.getChaptername()));
                            }
                        }
...
//                            indexFragment.chapterlistAutoBuy(mAid);
    ...
    public void beforeAutoBuy(){
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("提示").setMessage("开启自动购买后,系统将为您自动订阅该书全部VIP章节?再次点击退订!").setPositiveButton("自动订阅", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
            ...
            public void onClick(DialogInterface dialogInterface, int i) {
                ...
                if (type == 2) {                    ReadActivity.this.finish();
                }
    ...
    private void openDownloadBookDialog(ChapterBlockModel chapterBlockModel) {
        ...
        mDownloadBookDialog.setDialogListener(new DownloadBookDialog.DialogListener() {
            ...
            public void itemClick(ChapterBlockInfo chapterBlockInfo) {
                ...
                isSelected = mDownloadBookDialog.getChecked();
                if (chapterBlockInfo != null) {
                    ...
                    float allMoney = (float)(account + luckymoney);

                    if( (sumPrice > account && !isSelected) || (isSelected && sumPrice > allMoney) ){

                        openRechargeDialog();

                    } else {
                        chapterblockdownload(chapterBlockInfo,isSelected);
    ...
    public Button btn_luckymoney;

    /**
     * 多章下载
     */
    public void chapterblockdownload(final ChapterBlockInfo chapterBlockInfo,boolean isSelected) {
        ...
        log("isSelected=============="+isSelected);//是否使用阅读币
        int buymode = 0;
        if(isSelected){
            buymode = 1;
        }else{
        ...
        String url = String.format(Locale.CHINA, "%s/chapterblockdownload_luckymoney.php?aid=%d&uid=%d&fromOrder=%d&toOrder=%d&sign=%s&buymode=%d",
                SERVICE_URL, mAid, mUid, fromOrder, toOrder, sign,buymode);
    ...
    private void getLuckyMoney(){
        ...
        String url = String.format(Locale.CHINA, "%s/getluckymoney.php?act=getnum&uid=%d", SERVICE_URL,  mUid);
    ...
    public static double getLuckyMoneyNum(){
        return luckymoney;
    }
