...
public class PageFactory {
    ...
    private PageFactory(Context context) {
        ...
        lineSpace = context.getResources().getDimension(R.dimen.reading_line_spacing3);
...
//
    ...
    public void onDraw(Bitmap bitmap, List<String> m_lines, Boolean updateCharter, int index) {
        ...
        if (m_lines.size() > 0) {
            ...
            ReadActivity.log("最大宽度:"+mVisibleWidth);
            ReadActivity.log("最大宽度measureMarginWidth:"+measureMarginWidth);
            ReadActivity.log("最大宽度mWidth:"+mWidth);
            ReadActivity.log("最大宽度m_fontSize:"+m_fontSize);
            /// 循环绘制每一行数据
            for (int d = 0; d < strLen; d++) {
                ...
                if (!drawTextStr.equals("")) {
                    ...
                    //获取下一行文字
                    if(drawTextStr.startsWith("\u3000\u30000")){
                        float kgLength = mPaint.measureText("空格");
                        drawTextStr = drawTextStr.replace("\u3000\u30000", "");
                        c.drawText(drawTextStr, measureMarginWidth + kgLength, y, mPaint);
                    }else{
                ...
                if (d > 0) {
                    ...
                    indexLineLen = mWidth - 100 ;

                }

                if (panDuanPic(d, upSs, strLine) && !strLine.contains("##{0}")) {
...
//            result = drawTextStr.replace(ss, "\u0020\u0020");
            ...
            if(drawTextStr.startsWith("&*#    $$$#{}")){
                drawTextStr = drawTextStr.replace("#{}", "\u3000\u3000");
            }

            if (drawTextStr.startsWith("&*#    $$$")) {
                drawTextStr = drawTextStr.replace("&*#    ", "");
            }else if(drawTextStr.startsWith("&*#")){
                drawTextStr = drawTextStr.replace("&*#    ", "");
            }
...
//        trPage.setLines(getPreLines());
        ...
        ReadActivity.log("!!!!position==="+mBookUtil.getPosition());
    ...
    public List<String> getPreLines() {
        ...
        while (par != null) {
            ...
            for (int i = 0; i < par.length; i++) {
                ...
                if((width + widthChar) > mVisibleWidth) {
                    ReadActivity.log("当前line#$*:" + line);
                    ReadActivity.log("当前line#$*长度:" + width);
                    String nline = disposeStr(line);
                    width = mPaint.measureText(nline);
                    ReadActivity.log("处理后nline#$*:" + nline);
                    ReadActivity.log("处理后nline#$*长度:" + width);
                }
        ...
        while (mBookUtil.next(true) != -1) {
            ...
            if ((String.valueOf(word)).equals("\r") && (String.valueOf((char) mBookUtil.next(true))).equals("\n")) {
                ...
                if (!line.isEmpty()) {
                    ...
                    /// 保存数据到集合
                    linesList.add(line);
                    ReadActivity.log("BeforeNext2:"+line);
                    ReadActivity.log("next2:"+(String.valueOf((char) mBookUtil.next2(true))));
                    if( (String.valueOf((char) mBookUtil.next2(true))) != "$" && (String.valueOf((char) mBookUtil.next2(true))) != "#"){
                        line = "&*#    ";
                        width = mPaint.measureText("空格");
                    }else{
                        line = "";
                        width = 0;
                    }
