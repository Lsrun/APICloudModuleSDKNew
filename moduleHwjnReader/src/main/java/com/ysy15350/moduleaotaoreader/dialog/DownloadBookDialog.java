...
public class DownloadBookDialog extends Dialog {
    ...
    private void initView() {
        ...
        btn_luckymoney = mHolder.getView(R.id.btn_luckymoney);
        btn_luckymoney.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                isSelected = isChecked;
                ReadActivity.log("isSelected++++++++"+isSelected);
                checkAccount(mSelectChapterBlockInfo);
            }
        });
    ...
    private void checkAccount(ChapterBlockInfo chapterBlockInfo) {
        if (chapterBlockInfo != null) {
            ...
            /// 需要的总金额
            double luckymoney = ReadActivity.luckymoney;
            if ((chapterBlockInfo.getSumprice() > account&&!isSelected) || (isSelected && chapterBlockInfo.getSumprice()>((float)account + luckymoney) )){
